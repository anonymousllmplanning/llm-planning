...
reference ordering moves from the native chain. Table~\ref{tab:appendix-critical-path-buckets}
first summarizes the three replay-filtered coverage buckets with task-weighted
critical-path statistics, and Table~\ref{tab:appendix-critical-path} reports the
row-level reordering diagnostic over increasingly strict retained-ordering
thresholds. The $\geq 2$ threshold is the multi-order-rich subset used in the
main flexible-order diagnostics.
...
\toprule
Task bucket & Tasks & Native mean & Native median & Critical mean & Critical median & Parallelism mean \\
\midrule
All tasks & 165 & 7.78 & 7.0 & 6.45 & 6.0 & 11.6\% \\
Native-only & 107 & 6.35 & 6.0 & 6.35 & 6.0 & 0.0\% \\
Exactly-one & 4 & 5.50 & 5.5 & 4.50 & 4.5 & 19.0\% \\
Multi-order-rich & 54 & 10.80 & 9.0 & 6.81 & 6.5 & 33.9\% \\
\bottomrule
\end{tabular}
\caption{Task-weighted reference-validated critical-path profile by replay-filtered coverage bucket. Critical path is computed from the conservative partial order induced by final Augmented GAIA references, so native-only tasks preserve the native chain by definition.}
\label{tab:appendix-critical-path-buckets}
\end{table}
